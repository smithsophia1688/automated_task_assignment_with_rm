...
class MultiAgentFiveAgentEnv: #There is some henious code here but it like, works (would also be easy to clean up. )
    ...
    def check_if_at_button(self, pos,  agent_id):
        ei = self.agent_events_dict[agent_id]['ei']
        button_pos = self.env_settings['button_locs'][ei]
        if pos == button_pos:
            if self.flags[ei] == False:
                return True
            else:
                return False
        else:
    ...
    def get_mdp_label(self, s, s_next, u):
        ...
        pos_dict = {}
        for i in range(self.num_agents):
            rowi, coli = self.get_state_description(s_next[i])
            pos_dict[i] = (rowi, coli)
        
        for i in range(self.num_agents):
            pos = pos_dict[i]
            if self.check_if_at_button(pos, i):
                l.append(self.agent_events_dict[i]['ei']) #append ei
        if self.flags['e1']: 
...
def play():
    ...
    rm_string = os.path.join(base_file_dir, 'automated_task_assignment_with_rm', 'data', 'saved_reward_machines', 'five_agent', 'five_agent_team_rm_early_consequences.txt')
    ...
    game = MultiAgentFiveAgentEnv(rm_string, n, env_settings)
