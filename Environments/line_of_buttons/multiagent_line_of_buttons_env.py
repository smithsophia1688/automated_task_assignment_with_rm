...
class MultiAgentCraftingEnv: #There is some henious code here but it like, works (would also be easy to clean up. )
    ...
    def __init__(self, rm_file, num_agents, env_settings):
        ...
        self.flags= {'b1': False, 'b2': False, 'b3': False, 'b4': False, 'b5': False, 'b6': False, 'b7': False, 'b8': False, 'b9': False, 'b10': False, 'failed' : False}
    
        self.agents = [x for x in range(self.num_agents)]
        self.buttons = ['b'+ str(i+1) for i in range(self.num_buttons)]
        self.agent_was_at_button = { }
        for a in self.agents:
            self.agent_was_at_button[a] = self.flags.copy()
    ...
    def check_agent_was_at_button(self, a, bi):
        print(self.agent_was_at_button)
        return self.agent_was_at_button[a][bi]

    def get_mdp_label(self, s, s_next, u):
        l = []
        # go through each button 
        for a in self.agents: # a = 0, 1, 2, ... 
            for bi in self.buttons:
                if self.flags[bi] != True:
                    row, col = self.get_state_description(s_next[a])
                    if (row, col) == self.env_settings['button_locs'][bi]: # will only happen for at most one button per agent
                        if self.button_ready(bi):
                            if self.check_agent_was_at_button(a, bi):
                                l.append(bi + '_on')
                                self.flags[bi] = True # not implementing any "leaving" yet. also don't care how many times a button is pressed once it is pressed. 
                            else:
                                abi = str(a+1) + bi
                                l.append(abi)
                            self.agent_was_at_button[a][bi] = True
                    else:
                        self.agent_was_at_button[a][bi] = False
                                
        return l
                
    def is_complete(self):
        last_button = self.buttons[-1]
        return self.flags[last_button]
...
def play():
    ...
    env_settings['Nr'] = 10
    env_settings['Nc'] = 10
    env_settings['initial_states'] = [20]* n
    ...
    game = MultiAgentCraftingEnv(rm_string, n, env_settings)
