...
def get_relation(event_space, rm):
    ...
    Follows definition of equivalence relation outlined in Neary' paper (above Def 2)
...
def project_rm(event_space, rm):
    ...
    Follows procedure outlined in Definition 2 of "RM for Cooperative MARL" 
...
def get_accident_avoidance_rm_less(individual_rm, accident_set, rm, check = False):
                            ...
                            if rm.delta_r[v][x] < 0:
                                ...
                                if e in td.keys():
                                    if td[e] != dead_state:
                                        if check:
                                            return False
                                        #print("this was non deterministic yes?")
                                        #print("   ", e, v, u1 )
                                        #print('    ', td.keys())
...
def get_accident_avoidance_rm_less_2(individual_rm, accident_set, rm, check = False):
    #print("Its about to get crazy buckle up. ")
    dead_state = -1
    dead_reward = -1
    acc_rm_u = individual_rm.U.copy()
