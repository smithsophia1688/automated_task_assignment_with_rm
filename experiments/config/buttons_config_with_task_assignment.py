...
def buttons_config_ta(num_times, num_agents, rm_files):
    """
    ...
    print(base_file_path)
    joint_rm_file = os.path.join(base_file_path, 'automated_task_assignment_with_rm', 'data', 'saved_reward_machines', 'buttons', 'team_buttons_rm.txt')
    
    print("JOINT", joint_rm_file)
    local_rm_files = [] #add a config attribute to "what type of decomp" we are learningn on?

    for f in rm_files:
        full_file = base_file_path + '/automated_task_assignment_with_rm/data/saved_reward_machines/buttons/' + f
